...
\texttt{.get()} devolve o valor encontrado ou \texttt{None} quando a chave não existe. Isso evita erro ao buscar um ID ausente. No cadastro, o programa rejeita ID duplicado logo após sua digitação. Na atualização, o ID não é modificável: nome, responsável, localização, tipo e criticidade podem mudar, mas a identidade do ativo é preservada. Antes de alterar o objeto, todos os valores novos são validados; portanto, uma entrada inválida não deixa o ativo parcialmente modificado. Na exclusão, a tela mostra o alvo e exige a palavra \texttt{SIM}; ao remover o ativo, sua lista de vulnerabilidades também desaparece junto.
