...
Os 20 testes verificam cadastro, busca, ID duplicado, tipo inválido, filtros, atualização e confirmação de exclusão de ativos. Também verificam CVE, CVSS e data inválidos, CVE duplicado, atualização e remoção de vulnerabilidades, exigência de verificação para correção e proteção do JSON corrompido. Há testes específicos para provar que uma atualização inválida não altera somente uma parte do registro e que um JSON com ID incoerente ou tipo inválido é recusado. Os testes usam arquivos temporários e não alteram \texttt{dados/inventario.json}.
