...
\cardsection{S2\_18 -- Conferência manual e correções}

O programa pode ser conferido sem arquivo de teste separado, percorrendo o fluxo principal no terminal. Execute \texttt{python app.py}, cadastre um ativo, tente repetir seu ID, vincule uma vulnerabilidade, consulte, atualize e remova esse registro. Feche e abra o programa para confirmar que o JSON preservou os dados. Por fim, informe um CVSS fora de 0 a 10 e verifique que o menu continua aberto com uma mensagem clara.

Essa conferência não substitui o raciocínio sobre cada regra: ela torna visível que validações, CRUD e persistência se conectam no uso real. Antes de apresentar, é recomendável usar dados de exemplo e confirmar se o arquivo JSON está legível.
