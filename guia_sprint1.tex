...
Um ativo possui os campos ID, nome, responsável, localização, tipo, importância para a segurança e uma lista de vulnerabilidades. O ID é um número inteiro único. Ele identifica o ativo sem ambiguidade, mesmo que duas pessoas usem nomes parecidos. Na versão final, os dados são organizados em objetos simples, mas o dicionário externo continua usando o ID como chave.

\begin{lstlisting}[language=Python,caption={Formato dos dados na memória}]
ativo_servidor = Ativo(
    10, "SRV-ARQUIVOS", "TI", "Sala 02",
    TipoAtivo.SERVIDOR, "Alta"
)
vulnerabilidade = Vulnerabilidade(
    "CVE-2021-44228", "CWE-502", 10.0,
    "Falha conhecida", "NVD", "2026-09-25",
    "Alto", "Critica", "Atualizar", "Aberta", ""
)
ativo_servidor.vulnerabilidades.append(vulnerabilidade)

ativos = {10: ativo_servidor}
\end{lstlisting}

O dicionário externo usa o ID como chave. Assim, a pergunta ``já existe um ativo com este ID?'' pode ser respondida diretamente com \texttt{if identificador in ativos}. Cada chave aponta para um objeto \texttt{Ativo}, cujos atributos têm nomes claros, como \texttt{ativo.responsavel}. A lista \texttt{vulnerabilidades} pertence a esse objeto porque um ativo pode ter nenhuma, uma ou várias vulnerabilidades.

\section{Fluxo do programa}

O ponto de entrada \texttt{iniciar()}, em \texttt{app.py}, chama \texttt{carregar\_ativos()} para recuperar o JSON salvo e só então inicia \texttt{executar\_programa()}. Se o arquivo estiver inválido, o programa encerra sem sobrescrevê-lo. Depois disso, o menu permanece em um laço até a saída ser escolhida.
...
\begin{enumerate}
  ...
  \item \textbf{Ler:} menus e categorias usam as setas para cima e para baixo com \texttt{Enter}; campos de texto recebem o dado digitado.
  ...
  \item \textbf{Sair:} a opção 0 tenta salvar. Se houver falha, a pessoa pode tentar novamente ou sair conscientemente sem gravar as últimas alterações.
\end{enumerate}
...
\projeto
O menu utiliza \texttt{if/elif/else} para chamar a função correspondente à opção. A busca decide entre encontrado e não encontrado. A remoção só ocorre após uma confirmação explícita: a opção inicial é \texttt{Cancelar} e a pessoa precisa selecionar \texttt{Confirmar}. Cada situação é uma pergunta que produz verdadeiro ou falso.
...
\projeto
\texttt{cadastrar\_ativo()}, \texttt{consultar\_por\_id()}, \texttt{salvar\_ativos()} e \texttt{carregar\_ativos()} separam responsabilidades. \texttt{executar\_programa()} coordena o fluxo, sem precisar conter todos os detalhes de cadastro e arquivo.
...
\projeto
\texttt{TipoAtivo} possui seis categorias. \texttt{escolher\_tipo\_ativo()} mostra uma lista navegável: as setas para cima e para baixo mudam a opção e \texttt{Enter} confirma. O objeto usa a enumeração durante a execução; ao salvar, ela é convertida para um valor simples do JSON e, ao carregar, volta a ser uma categoria válida.
...
\projeto
\texttt{carregar\_ativos()} trata problemas de leitura, \texttt{JSONDecodeError} e \texttt{DadosInvalidosError}. A exceção personalizada \texttt{DadosInvalidosError} representa uma regra do programa: o conteúdo carregado precisa ter a estrutura esperada de ativos e vulnerabilidades. Se o arquivo estiver corrompido ou tiver formato inválido, o usuário é avisado e o programa é encerrado antes de salvar qualquer coisa. Assim, o arquivo com problema não é sobrescrito por engano.
...
Uma classe é um molde para representar dados relacionados. Neste trabalho, \texttt{Ativo} reúne ID, nome, responsável, localização, tipo, importância para a segurança (também chamada de criticidade do ativo), vulnerabilidades e histórico. Essa importância informa quanto a indisponibilidade ou o comprometimento daquele ativo afeta a organização; não é a severidade de uma vulnerabilidade. \texttt{Vulnerabilidade} reúne CVE, CWE, CVSS, descrição, fonte, data, impacto, prioridade, tratamento, status e verificação.
...
No terminal do Windows, o menu principal e categorias como tipo de ativo, importância, prioridade e status são escolhidos com as setas para cima e para baixo e confirmados com \texttt{Enter}. O programa mostra o item atualmente selecionado e converte a escolha para o texto padronizado antes de salvar. Nos formulários, a seta para a esquerda retorna ao campo anterior; a seta para a direita avança apenas quando aquele campo já possui uma resposta válida. Ao voltar, o valor anterior continua no campo para ser corrigido com \texttt{Backspace} ou complementado. Assim, não é necessário digitar acentos nem lembrar a escrita exata de cada opção.
...
\texttt{.get()} devolve o valor encontrado ou \texttt{None} quando a chave não existe. Isso evita erro ao buscar um ID ausente. No cadastro, o programa rejeita ID duplicado logo após sua digitação. Na atualização, o ID não é modificável: nome, responsável, localização, tipo e importância do ativo para a segurança podem mudar, mas a identidade do ativo é preservada. O termo \emph{criticidade} é usado internamente porque o checklist pede valores padronizados, mas a tela explica que ele representa a importância do ativo, e não a severidade de uma falha. Antes de alterar o objeto, todos os valores novos são validados; portanto, uma entrada inválida não deixa o ativo parcialmente modificado. Na exclusão, a tela mostra o alvo e apresenta uma escolha com \texttt{Cancelar} selecionado inicialmente. Só a opção \texttt{Confirmar} remove o ativo e sua lista de vulnerabilidades.
...
nota = float(cvss.replace(",", "."))
if not 0.0 <= nota <= 10.0:
    raise DadosInvalidosError("CVSS deve estar entre 0.0 e 10.0.")
\end{lstlisting}

O \texttt{raise} interrompe apenas a operação inválida e a interface mostra uma mensagem compreensível. O menu continua aberto. No CVSS, a vírgula decimal também é aceita: \texttt{9,8} é normalizado para \texttt{9.8} antes da conversão, mas valores fora de 0.0 a 10.0 continuam rejeitados. CVE, CWE, CVSS e a data da fonte são conferidos assim que são digitados, evitando que a pessoa responda todas as perguntas para descobrir um erro inicial. O sistema também exige uma verificação antes de cadastrar ou mudar o status para corrigida, pois dizer que algo foi corrigido sem evidência não é suficiente. Como na atualização de ativos, os novos dados são todos validados antes de modificar a vulnerabilidade.

Para consultar sem decorar códigos, a pessoa pode abrir a lista geral e escolher uma vulnerabilidade com as setas; também pode informar o ID de um ativo para ver apenas suas vulnerabilidades. O filtro avançado permanece disponível quando a busca precisa combinar status, prioridade ou outros critérios. O CVE não é editado depois do cadastro: isso preserva a identidade do registro e evita criar duplicidade por engano.
...
Essas regras aparecem em funções de validação, como \texttt{validar\_texto()}, \texttt{validar\_cve()}, \texttt{validar\_cwe()}, \texttt{validar\_cvss()} e \texttt{validar\_data\_fonte()}. Quando uma regra falha, elas usam \texttt{DadosInvalidosError}; a interface captura essa exceção e apresenta uma mensagem curta. Ao abrir o JSON, as mesmas regras conferem os dados salvos e também confirmam que a chave do dicionário é igual ao ID interno do ativo. Se houver incoerência, o arquivo não é sobrescrito.
...
O conjunto atual possui 33 testes. Eles verificam cadastro, busca, ID duplicado, tipo inválido, filtros, atualização e confirmação de exclusão de ativos. Também verificam CVE, CVSS e data inválidos, CVE duplicado, atualização e remoção de vulnerabilidades, exigência de verificação para correção e proteção do JSON corrompido. Há testes específicos para provar que uma atualização inválida não altera somente uma parte do registro, que vírgula é aceita no CVSS, que os atalhos de voltar e cancelar funcionam e que um JSON com ID incoerente ou tipo inválido é recusado. Os testes usam arquivos temporários e não alteram \texttt{dados/inventario.json}.
...
\begin{enumerate}
  ...
  \item Consulte pela lista e depois informe o ID do ativo para mostrar que a vulnerabilidade está vinculada a ele.
  \item Atualize o responsável ou o status da vulnerabilidade; ao escolher corrigida, informe uma verificação.
  \item Tente repetir um ID, usar um CVSS inválido e depois informe \texttt{9,8} para mostrar a validação decimal.
  \item Remova uma vulnerabilidade, primeiro mantendo \texttt{Cancelar} e depois escolhendo \texttt{Confirmar}.
